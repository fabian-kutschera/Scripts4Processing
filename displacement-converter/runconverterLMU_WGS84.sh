...
bathy=gebco_2022_n40.0_s36.0_w23.0_e29.0.nc

#convert to smaller data set and keep .h5 - SKIP?
#python /import/freenas-m-05-seissol/kutschera/HIWI/Scripts/Script4Processing/extractDataFromUnstructuredOutput.py /import/freenas-m-05-seissol/kutschera/MAthesis_fkutschera/simulations/Samos_final_Fra_v1/${input}-surface.xdmf --Data u1 u2 u3 --idt $(seq 0 30) --backend hdf5
...
/import/freenas-m-05-seissol/SamoaRelated/displacement-converter/build/displacement-converter -i WGS84_${input}_resampledUVW-surface.xdmf -o $output -b $bathy --dx $dx --dy $dx -x25 -w3 -y36.5 -z 3 --dt 1

#apply tappering on the domain boundary for a smooth transition to 0
#python tapperNetcdf.py $output --nHann 100

#convert netcdf file to tt3 (here using 4 ranks)
#python convert_netcdf_tt3.py tapered_$output --MP 4 --dow 2
